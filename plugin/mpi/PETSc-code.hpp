...
void varfBem(const typename fes1::FESpace*& PUh, const typename fes2::FESpace*& PVh, bool same, int VFBEM, Stack stack, const list<C_F0>& bargs, const Data_Sparse_Solver& ds, Dmat* B) {
    ...
    if (VFBEM == 1) {
        ...
        HtoolCtx<P1,MeshBemtool>* ctx = new HtoolCtx<P1,MeshBemtool>;
        ctx->dof = dof;
        ctx->mesh = mesh;
        ctx->node = node;
        ff_BIO_Generator<PetscScalar,P1,Mesh1>(ctx->generator,Ker,*ctx->dof,alpha);
        PetscContainer ptr;
        ...
        PetscContainerSetUserDestroy(ptr, DestroyHtoolCtx<P1,MeshBemtool>);
        ...
        ff_POT_Generator<Dmat,PetscScalar,P1,MeshBemtool,Mesh1>(B,Pot,*dof,*mesh,node_output,ds.sparams,p1,p2,MPI_COMM_NULL);
        delete dof;
        delete mesh;
        delete node;
    }
